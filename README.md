...
```
#### 接口
接口的设计主要参照了RESTful API的规范（不知道自己理解的是不是正确😂 ，可能存在理解偏差所导致的设计不规范）。

从我个人的理解角度看来，一个URL即是指向一个对应资源的路标，那么该资源就是目的地；请求所使用的HTTP方法就类似于去到目的地的过程中的交通工具；请求的返回就类似于最终的结果：
1. 到达目的地，看到美丽的风景，那么请求就是成功的，返回2XX状态及相应的返回值
2. 要去的目的地已经不在那里了，换到另外一个位置了，那么就是说该请求的URI发生了重定向，返回3XX状态
3. 由于自身原因（身体不舒服，妈妈叫回家吃饭了，等等），无法去到目的地了，那么就类似于客户端请求失败，返回4XX状态
4. 目的地因为某种原因不在那里了，而且其他地方也找不到，那么就类似于服务端的异常，返回5XX状态
